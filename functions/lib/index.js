 ...
 */
async function getCollectionDocsForBarrios(collectionName, activeBarrioOrgs, applyExtra) {
...
exports.dailyNotifications = functions
    ...
    {
        const servicesSnap = await getCollectionDocsForBarrios("c_servicios", activeBarrioOrgs, (q) => q
    ...
    {
        const actSnap = await getCollectionDocsForBarrios("c_actividades", activeBarrioOrgs, (q) => q
            .where("date", ">=", admin.firestore.Timestamp.fromDate(today))
            .where("date", "<=", admin.firestore.Timestamp.fromDate(in14Days)));
...
exports.weeklyNotifications = functions
    ...
    {
        const deceasedMembersQuery = await getCollectionDocsForBarrios("c_miembros", activeBarrioOrgs, (q) => q.where("status", "==", "deceased"));
    ...
    const convertsTrace = buildNotificationTrace("weeklyNotifications", "converts");
    {
        ...
        const [membersSnap, friendsSnap, convertInfoSnap] = await Promise.all([
            getCollectionDocsForBarrios("c_miembros", activeBarrioOrgs, (q) => q.where("baptismDate", ">=", cutoffTs)),
    ...
    const missionaryWorkTrace = buildNotificationTrace("weeklyNotifications", "missionaryWork");
    {
        ...
        const [assignmentsSnap, investigatorsSnap, recentConvertMembersSnap] = await Promise.all([
            getCollectionDocsForBarrios("c_obra_misional_asignaciones", activeBarrioOrgs, (q) => q.where("isCompleted", "==", false)),
            getCollectionDocsForBarrios("c_obra_misional_investigadores", activeBarrioOrgs, (q) => q.where("status", "==", "active")),
            getCollectionDocsForBarrios("c_miembros", activeBarrioOrgs, (q) => q.where("baptismDate", ">=", mwCutoffTs)),
        ]);
...
exports.syncOnInvestigatorsWrite = makeDataSyncFn("c_obra_misional_investigadores", "c_obra_misional_investigadores/{docId}");
exports.syncOnNewConvertFriendsWrite = makeDataSyncFn("c_obra_misional_amigos_conversos", "c_obra_misional_amigos_conversos/{docId}");
exports.syncOnHealthConcernsWrite = makeDataSyncFn("c_observaciones_salud", "c_observaciones_salud/{docId}");
exports.syncOnBirthdaysWrite = makeDataSyncFn("c_cumpleanos", "c_cumpleanos/{docId}");
exports.syncOnBaptismsWrite = makeDataSyncFn("c_bautismos", "c_bautismos/{docId}");
exports.syncOnFsTrainingsWrite = makeDataSyncFn("c_fs_capacitaciones", "c_fs_capacitaciones/{docId}");
exports.syncOnFsAnnotationsWrite = makeDataSyncFn("c_fs_anotaciones", "c_fs_anotaciones/{docId}");
exports.syncOnConvertsWrite = makeDataSyncFn("c_conversos", "c_conversos/{docId}");
...
exports.requestDataSyncSignal = functions
    ...
    if (requestedBarrioOrg !== callerBarrioOrg) {
        throw new functions.https.HttpsError("permission-denied", "No puedes disparar señales de sync para otro barrio.");
    }
    await (0, data_sync_publisher_1.publishSyncSignal)(firestore, messaging, functions.logger, {
